...
新增六张英文 SVG，可以在研究页的 **System drawings** 查看，或下载整张比较板。关键点图另附编号版和 JSON 数据。

| 图         | 画的是什么                       | 数据精度                                                    |
| ---------- | -------------------------------- | ----------------------------------------------------------- |
| 5 点       | 两眼中心、鼻子、两嘴角，用于对齐 | InsightFace 的真实目标模板坐标；虚线是说明用的辅助线        |
| 68 点      | 下颌、眉、眼、鼻、唇轮廓         | dlib 标准编号和连接；脸型坐标为手绘示意                     |
| 106 点     | 更细的五官采样                   | 根据 InsightFace 官方链接的编号图人工近似描点；不虚构网格边 |
| 468 点     | 脸部表面的三角网格               | MediaPipe 官方 canonical OBJ 正面投影，采用官方连接         |
| 478 点示意 | 468 点基础加两侧虹膜各 5 点      | 基础 468 点为官方数据；另外 10 点的编号有依据，位置为示意   |
| 19 类区域  | 皮肤、五官、头发及背景和配饰     | CelebAMask-HQ 的真实类别编号；形状为原创示意                |

五点对齐不等于用五个点判断一个人的身份。现代验证流程通常把对齐后的脸送入特征提取模型，再比较两张脸的特征向量。68、106 和密集网格也不天然构成身份验证器。这里的图用于比较**人脸如何被描述、定位和分区**。

106 点图中：轮廓 0–32，眼部 33–42 / 87–96，眉部 43–51 / 97–105，嘴部 52–71，鼻部 72–86。编号不都沿轮廓连续递增，所以不能直接依次连线。这是根据官方链接图的整理。

完整链接与绘图方法见 [sources](references/systems/SOURCES.md)。特别注意：468 的官方模型不包含虹膜的 10 个点，478 图中这些点以橙色标明，不能当成模型运行的结果。
